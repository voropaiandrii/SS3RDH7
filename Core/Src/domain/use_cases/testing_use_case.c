...
static char currentTestFilename[FILE_UTILS_GENERATED_NAME_LENGTH_BYTES];
...
void sdcardTestingUseCase() {
	...
	for(uint32_t i = 0; i < TESTING_BUFFER_SIZE; i++) {
	...
	if(BSP_PlatformIsDetected() == SD_PRESENT) {
		if(retSD == 0) {
			...
			if(fresult == FR_OK) {
				...
				fresult = f_open(&MyFile, (char*)currentTestFilename, FA_OPEN_APPEND | FA_WRITE);
				if(fresult == FR_OK) {
					...
					fresult = f_write(&MyFile, testWritingBuffer, TESTING_BUFFER_SIZE, (void *)&writtenBytes);
					if(fresult == FR_OK) {
						printt("Written bytes: %lu, buffer length: %lu\n", writtenBytes, TESTING_BUFFER_SIZE);
						fresult = f_sync(&MyFile);
						if(fresult == FR_OK) {
							fresult = f_read(&MyFile, testReadingBuffer, TESTING_BUFFER_SIZE, (void *)&readBytes);
							if(fresult == FR_OK) {
								printt("Read bytes: %lu, buffer length: %lu\n", readBytes, TESTING_BUFFER_SIZE);
								for(uint32_t i = 0; i < TESTING_BUFFER_SIZE; i++) {
